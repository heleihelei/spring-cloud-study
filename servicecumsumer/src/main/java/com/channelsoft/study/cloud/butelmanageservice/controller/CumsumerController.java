...
public class CumsumerController {
    ...
    public List<ButelNumPo> list() {
        try {
            List<ButelNumPo> list = this.restTemplate.getForObject("http://localhost:8081/butel/list", List.class);
            return list;
    ...
    public List<ButelNumPo> retListTimeout() {
        try {
            List<ButelNumPo> list = this.restTemplate.getForObject("http://localhost:8081/butel/list-timeout", List.class);
            return list;
        } catch (RestClientException e) {
            logger.info("进入RestClientException..." + e.getMessage());
            logger.info(e.getMessage(),e);
    ...
    public List<ButelNumPo> retListException() {
        try {
            List<ButelNumPo> list = this.restTemplate.getForObject("http://localhost:8081/butel/exception", List.class);
            //下面不能直接获取po对象，会报异常
    ...
    public List<ButelNumPo> retList() {
        try {
            ButelNumPo[] list = this.restTemplate.getForObject("http://localhost:8081/butel/list", ButelNumPo[].class);
            //下面不能直接获取po对象，会报异常
            for (ButelNumPo butelNumPo : list) {
                logger.info(butelNumPo.toString());
            }
    ...
    public List<ButelNumPo> retList1() {
        try {
            ResponseEntity<List<ButelNumPo>> responseEntity = restTemplate.exchange("http://localhost:8081/butel/list", HttpMethod.GET, null, new ParameterizedTypeReference<List<ButelNumPo>>() {
            });
    ...
    public  AjaxPo<List<ButelNumPo>> retList2() {
        try {
            ResponseEntity<AjaxPo<List<ButelNumPo>>> responseEntity = restTemplate.exchange("http://localhost:8081/butel/list-ajax", HttpMethod.GET, null, new ParameterizedTypeReference<AjaxPo<List<ButelNumPo>>>() {
            });
